...
대화 형식의 예제에는 여러 개의 assistant 역할 메시지가 포함될 수 있습니다. 기본적으로 파인튜닝은 하나의 예제 내의 모든 assistant 메시지에 대해 학습합니다. 특정 assistant 메시지에 대한 파인튜닝을 건너뛰려면 weight 키를 추가하여 해당 메시지에 대한 학습을 비활성화할 수 있습니다. weight의 허용 값은 현재 0 또는 1입니다.
...
- 모델이 수렴하지 않는 것으로 보이는 경우: 학습률 승수 증가
